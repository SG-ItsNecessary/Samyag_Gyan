const articles = [
  {
    ...
    "prelude_body": "It ties digital India with delivery on the ground. You'll see governance come alive in numbers. Also, it's a gem for GS2 introductions.",
    "content": [
      {
      ...
      },
      {
        ...
        "content": [
          ...
          },
          {
            ...
            "article_id": 1,
            "language": "en",
            "q": "What are the key components of DPI?",
            "a": "Key components include Aadhaar (biometric identity system for 1.3 billion people), UPI (Unified Payments Interface for instant digital payments), DigiLocker (cloud-based document storage), and e-Sign (digital signature framework). These work together to create a seamless digital ecosystem for citizens and businesses."
          },
          {
            ...
            "article_id": 1,
            "language": "en",
            "q": "How does DPI enable financial inclusion?",
            "a": "DPI allows citizens without traditional banking access to receive direct benefit transfers, make digital payments, and access government services. JAM Trinity (Jan Dhan accounts + Aadhaar + Mobile) has brought over 400 million unbanked Indians into the formal financial system, enabling them to receive subsidies worth ₹6 lakh crore annually."
          },
          {
            "prelims_question_id": 305,
            "prelims_answer_id": 405,
            "article_id": 1,
            "language": "en",
            "q": "What is the JAM Trinity?",
            "a": "JAM stands for Jan Dhan (zero-balance bank accounts for all), Aadhaar (unique identity for every resident), and Mobile (connectivity for digital transactions). This trinity has enabled direct benefit transfers worth ₹27 lakh crore since 2014, eliminating middlemen and reducing corruption in welfare delivery."
          },
          {
            "prelims_question_id": 306,
            "prelims_answer_id": 406,
            "article_id": 1,
            "language": "en",
            "q": "Which countries are adopting India's DPI model?",
            "a": "Countries in Africa (Ethiopia, Sierra Leone), Latin America (Argentina, Brazil), and Southeast Asia (Philippines, Singapore) are implementing India's DPI stack. The World Bank and IMF have recognized India's model as a blueprint for developing nations. Over 50 countries have expressed interest in adopting similar frameworks for digital governance."
          }
        ]
      },
      {
  ...
  },
  {
    ...
    "endRibbons": [
      ...
      },
      {
        ...
        "content": [
          {
            "prelims_question_id": 307,
            "prelims_answer_id": 407,
            "article_id": 2,
            "language": "en",
          ...
          {
            "prelims_question_id": 308,
            "prelims_answer_id": 408,
            "article_id": 2,
            "language": "en",
            "q": "What are the recent renewable energy initiatives?",
          ...
          {
            "prelims_question_id": 309,
            "prelims_answer_id": 409,
            "article_id": 2,
            "language": "en",
            "q": "What is the National Solar Mission?",
            "a": "Launched in 2010 as part of the National Action Plan on Climate Change, the National Solar Mission aims to establish India as a global leader in solar energy. It targets 100 GW of solar power capacity by 2022 (revised to 280 GW by 2030), with a focus on both large-scale grid-connected and off-grid solar installations."
          },
          {
            "prelims_question_id": 310,
            "prelims_answer_id": 410,
            "article_id": 2,
            "language": "en",
            "q": "What is PM-KUSUM scheme?",
            "a": "Pradhan Mantri Kisan Urja Suraksha evam Utthaan Mahabhiyan (PM-KUSUM) provides financial support to farmers for installing solar pumps and grid-connected solar power plants. It aims to add 30.8 GW of solar capacity by 2022, helping farmers reduce diesel dependency while generating additional income through surplus power sales."
          },
          {
            "prelims_question_id": 311,
            "prelims_answer_id": 411,
            "article_id": 2,
            "language": "en",
            "q": "What is the Production Linked Incentive (PLI) scheme for solar?",
            "a": "The PLI scheme for solar PV modules offers incentives worth ₹24,000 crore to boost domestic manufacturing. It aims to achieve 10 GW manufacturing capacity for integrated solar modules and reduce import dependency on China, which currently supplies 80% of India's solar equipment. The scheme is expected to generate investments of ₹17,000 crore."
          },
          {
            "prelims_question_id": 312,
            "prelims_answer_id": 412,
            "article_id": 2,
            "language": "en",
            "q": "What are the main challenges in India's renewable energy transition?",
            "a": "Key challenges include inadequate energy storage technology, land acquisition delays for large projects, insufficient transmission infrastructure, intermittency of solar and wind power, need for grid modernization, high upfront capital costs, and potential job losses in traditional coal industry. Financing large-scale projects remains a significant hurdle for developers."
          }
        ]
      },
      {
  ...
  articles.forEach((article) => {
    ...
    article.endRibbons.forEach((r) => {
      ...
      ribbon.addEventListener('click', () => {
        ...
        if (r.type === 'mindmap') {
          // Open mind map overlay with language and article info
          openMindMapOverlay(r.src, r.lang, article.title, article.publish_date, article.article_type);
        } else if (r.type === 'prelims') {
          // Open prelims overlay with article info
          openPrelimsOverlay(r.content, article.title, article.publish_date, article.article_type);
        }
      });
...
// -----------------------------
// Usage example (moved inside window.onload to avoid duplicate declaration)
// -----------------------------

window.onload = async function () {
  // ================================
  // ARTICLE ACCESS CONTROL (CRITICAL!)
  // ================================
  // Check subscription/trial BEFORE loading ANY article content
  const articleData = getArticleDataFromURL();

  if (articleData && articleData.articleType) {
    console.log('🔒 Checking access for:', articleData.articleType);

    // Verify access before proceeding
    const hasAccess = await verifyAndLoadArticle(articleData.articleType);

    if (!hasAccess) {
      console.log('❌ Access denied - stopping article load');
      // Don't load article content, subscription popup is already shown
      return;
    }

    console.log('✅ Access granted - proceeding with article load');
  }
  ...
  function createHighlightPalette(x, y) {
    ...
    // Add color buttons
    let currentHighlight = null; // Track current highlight span
    colors.forEach(color => {
      ...
      button.onclick = function () {
        ...
        // Start NEW 5-second timer: if tick not clicked, remove highlight and palette
...
// --- Step 1: Render hardcoded data immediately ---
  ...
  // --- Step 2: Try to fetch from backend API (will override if available) ---
  fetch('/api/articles')
    .then(res => res.json())
    .then(data => {
      if (Array.isArray(data) && data.length > 0) {
        ...
        console.warn("⚠️ API returned empty list, keeping dummy data.");
      }
    })
    .catch(err => {
      console.log("⚠️ API fetch failed, using dummy data:", err.message);
    });
  ...
  // Auto-open article from URL if slug is present (reusing articleData from top of function)
...
// ==================== MIND MAP OVERLAY FUNCTIONALITY ====================
let currentMindMapData = {};

function openMindMapOverlay(src, lang, title, publishDate, articleType) {
  ...
  // Store current mind map data for save functionality
  currentMindMapData = { src, lang, title, publishDate };

  // Fix image path: ensure it's relative to root (not current URL path)
  const imagePath = src.startsWith('/') ? src : '/' + src;
  image.src = imagePath;
  image.classList.remove("zoomed");
  mindmapTitle.textContent = lang === 'hi' ? 'माइंडमैप (Hindi)' : 'Mindmap (English)';
  viewer.classList.remove("hidden");

  // Update URL
  const slug = slugifyTitle(title);
  const isoDate = convertDateToISO(publishDate);
  const urlSuffix = lang === 'hi' ? 'upsc-mindmap-hi' : 'upsc-mindmap-en';

  let newPath;
  if (articleType === 'ethics' || articleType === 'essay') {
    newPath = `/${articleType === 'ethics' ? 'upsc-ethics' : 'upsc-essays'}/${slug}/${urlSuffix}`;
  } else {
    newPath = `/upsc-editorials/${isoDate}/${slug}/${urlSuffix}`;
  }

  history.pushState({ overlay: 'mindmap', lang, title, publishDate }, "", newPath);
}

// Close mind map overlay
function closeMindMapOverlay() {
  const viewer = document.getElementById("floating-image-viewer");
  viewer.classList.add("hidden");
  history.back(); // This will trigger popstate and restore previous URL
}

// ==================== PRELIMS OVERLAY FUNCTIONALITY ====================
let currentPrelimsData = {};

function openPrelimsOverlay(prelimsContent, title, publishDate, articleType) {
  const overlay = document.getElementById("prelims-overlay");
  const contentDiv = document.getElementById("prelims-overlay-content");
  const scrollIndicator = document.getElementById("prelims-scroll-indicator");
  const titleElement = document.querySelector(".prelims-overlay-title");

  // Store current prelims data
  currentPrelimsData = { prelimsContent, title, publishDate };

  // Update title to "Pointers for [Article Title]"
  titleElement.textContent = `Pointers for ${title}`;

  // Clear previous content
  contentDiv.innerHTML = '';

  // Calculate adaptive display: show first 50% of Q&A pairs
  const totalPairs = prelimsContent.length;
  const initialVisible = Math.ceil(totalPairs / 2);

  // Render all Q&A pairs
  prelimsContent.forEach((qna, index) => {
    const qaItem = document.createElement('div');
    qaItem.className = 'prelims-qa-item';
    qaItem.dataset.prelims_question_id = qna.prelims_question_id;
    qaItem.dataset.article_id = qna.article_id;

    const question = document.createElement('div');
    question.className = 'prelims-qa-question';
    question.textContent = `Q${index + 1}: ${qna.q}`;

    const answer = document.createElement('div');
    answer.className = 'prelims-qa-answer';
    answer.dataset.prelims_answer_id = qna.prelims_answer_id;
    answer.dataset.prelims_question_id = qna.prelims_question_id;
    answer.dataset.article_id = qna.article_id;
    answer.dataset.language = qna.language || 'en';
    answer.textContent = qna.a;

    qaItem.appendChild(question);
    qaItem.appendChild(answer);
    contentDiv.appendChild(qaItem);
  });
  ...
  contentDiv.addEventListener('scroll', () => {
    ...
    if (scrollPosition >= scrollHeight - 50) {
      scrollIndicator.classList.add('hidden');
    } else if (totalPairs > initialVisible) {
      scrollIndicator.classList.remove('hidden');
    }
  });
  ...
  let newPath;
  if (articleType === 'ethics' || articleType === 'essay') {
    newPath = `/${articleType === 'ethics' ? 'upsc-ethics' : 'upsc-essays'}/${slug}/upsc-prelims`;
  } else {
    newPath = `/upsc-editorials/${isoDate}/${slug}/upsc-prelims`;
  }

  history.pushState({ overlay: 'prelims', title, publishDate }, "", newPath);
}

// Close prelims overlay
function closePrelimsOverlay() {
  const overlay = document.getElementById("prelims-overlay");
  overlay.classList.add("hidden");
  history.back(); // This will trigger popstate and restore previous URL
}

// ==================== SAVE MINDMAP FUNCTIONALITY ====================
function saveMindMap() {
  const { src, lang, title, publishDate } = currentMindMapData;

  // Convert date to DD-MM-YYYY format
  const dateParts = publishDate.trim().split(' '); // "18 July 2025"
  ...
  const cleanTitle = title.replace(/[^a-zA-Z0-9\s\u0900-\u097F]/g, '').replace(/\s+/g, '_');

  // Create filename
  const langSuffix = lang === 'hi' ? 'hi' : 'en';
  const filename = `${formattedDate}_${cleanTitle}_${langSuffix}.png`;
...
// Close mind map with X button
document.getElementById("close-floating-image")?.addEventListener("click", closeMindMapOverlay);

// Close prelims with X button
document.getElementById("close-prelims-overlay")?.addEventListener("click", closePrelimsOverlay);

// Save mind map button
document.getElementById("save-mindmap-btn")?.addEventListener("click", saveMindMap);

// ESC key closes overlays
document.addEventListener("keydown", (e) => {
  if (e.key === "Escape") {
    const mindmapViewer = document.getElementById("floating-image-viewer");
    const prelimsOverlay = document.getElementById("prelims-overlay");

    if (!mindmapViewer.classList.contains("hidden")) {
      closeMindMapOverlay();
    } else if (!prelimsOverlay.classList.contains("hidden")) {
      closePrelimsOverlay();
    }
  }
});
...
window.addEventListener("popstate", (event) => {
  ...
  // Close any open overlays without calling history.back() again
  if (!mindmapViewer.classList.contains("hidden")) {
    mindmapViewer.classList.add("hidden");
  }
  if (!prelimsOverlay.classList.contains("hidden")) {
    prelimsOverlay.classList.add("hidden");
  }
});
...
function saveHighlightToBackend(userId, articleId, language, questionType, questionId, answerId, highlightedText) {
  ...
  .then(res => {
    if (!res.ok) {
      // Suppress errors when backend not running (testing mode)
